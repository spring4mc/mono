...
public interface TypeToken<T> extends Type, AnnotatedElement {
    ...
    static <T> TypeToken<T> ofMethodReturnType(Method method) {
        return convert(method.getGenericReturnType(), method.getAnnotatedReturnType());
    }
    ...
    static <T> TypeToken<T> ofFieldType(Field field) {
        return convert(field.getGenericType(), field.getAnnotatedType());
    }
     ...
     */
    static <T> TypeToken<T> convert(@NonNull Type type) {
    ...
    static <T> TypeToken<T> convertGeneric(Class<T> clazz) {
        ...
        return TypeTokenFactory.get().create(type, (AnnotatedElement) null);
    }
    ...
    static <T> TypeToken<T> capture(@NonNull Capturing<T> capturing) {
        return convert(capturing.getCapturedType(), capturing.getCapturedAnnotatedElement());
    }
